...
function List() {
  ...
  return (
    <div className="p-4 sm:p-6 mt-5 w-full">
      <h2 className="text-xl sm:text-2xl font-semibold mb-4 text-center">Transaction List</h2>
          ...
          <tbody>
            {filterList.map((cur) => (
              <tr key={cur.id} className="text-center hover:bg-gray-50 transition">
                <td className="px-2 sm:px-4 py-2 border">{cur.date}</td>
                <td className="px-2 sm:px-4 py-2 border">{cur.type}</td>
                <td className="px-2 sm:px-4 py-2 border">{cur.category}</td>
                <td className="px-2 sm:px-4 py-2 border">₹{cur.amount}</td>
                <td className="px-2 sm:px-4 py-2 border">{cur.desc}</td>
                <td className="px-2 sm:px-4 py-2 border">
                  <button
                    onClick={() => deleteTransaction(cur.id)}
                    className="bg-red-500 hover:bg-red-600 text-white px-2 sm:px-3 py-1 rounded text-sm sm:text-base"
                  >
                    Delete
                  </button>
                </td>
